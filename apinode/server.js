...
const sequelize = new Sequelize('mysql://avnadmin:AVNS_x0Njzz9bjlnHsELIEv4@laruche-api-cegeplimoilou-6ad2.aivencloud.com:20719/rucheDB?ssl-mode=REQUIRED');
...
const ProductLocation = sequelize.define('productLocation', {
	productLocationId: {
		type: Sequelize.INTEGER,
		primaryKey: true,
		autoIncrement: true,
...
app.post('/users', async (req, res) => {
	try {
		...
		const user = await User.create({ username, password: hashedPassword, firstName, lastName, privileges }); // Create a new user record
...
app.post('/locations/:locationId/productLocations', async (req, res) => {
	try {
		...
		const { productId} = req.body; // Extract the productId and isActive from the request body
		...
		const isActive = true;
		const productLocation = await ProductLocation.create({ locationId, productId, isActive }); // Create a new product location record
